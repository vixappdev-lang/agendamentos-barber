...
const MemberLogin = () => {
  ...
  const handleRegister = async () => {
    ...
    const { data, error } = await supabase.auth.signUp({
      email: getEmail(phone), password,
      options: { data: { full_name: fullName, phone } },
    });
    if (error) {
      if (error.message?.includes("already")) { toast.error("Este telefone já está cadastrado. Faça login."); setMode("login"); }
      else { toast.error("Erro ao criar conta."); }
      setLoading(false); return;
    }
    if (data.user) { toast.success("Conta criada com sucesso! Bem-vindo!"); navigate("/membro", { replace: true }); }
    setLoading(false);
  };

  return (
    <div className="min-h-screen w-full flex" style={{ background: t.pageBg, color: t.textPrimary, fontFamily: "'Montserrat', sans-serif" }}>
      ...
      <div className="hidden lg:block lg:w-[50%] xl:w-[52%] relative overflow-hidden" style={{ minHeight: "100vh" }}>
        <AnimatePresence mode="wait">
          <motion.div key={slideIndex} initial={{ opacity: 0, scale: 1.05 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.8 }} className="absolute inset-0">
            <img src={slides[slideIndex].img} alt="" className="w-full h-full object-cover" />
            <div className="absolute inset-0" style={{ background: `linear-gradient(135deg, ${t.pageBg} / 0.7) 0%, ${t.pageBg} / 0.3) 50%, ${t.pageBg} / 0.8) 100%)` }} />
          </motion.div>
        </AnimatePresence>
        <div className="absolute inset-y-0 right-0 z-20 pointer-events-none" style={{ width: "120px", background: `linear-gradient(to right, transparent, ${t.pageBg})` }} />

        <div className="relative z-10 flex flex-col justify-between p-10 xl:p-14 w-full">
          <a href="/" className="flex items-center gap-2.5">
            <div className="w-9 h-9 rounded-lg flex items-center justify-center" style={{ background: t.btnBg }}>
          ...
          <div>
            <AnimatePresence mode="wait">
              <motion.div key={slideIndex} initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: -20, opacity: 0 }}>
                ...
                <p className="text-base xl:text-lg" style={{ color: t.textSubtle }}>{slides[slideIndex].desc}</p>
              </motion.div>
            </AnimatePresence>
            <div className="flex gap-2 mt-8">
              {slides.map((_, i) => (
                <button key={i} onClick={() => setSlideIndex(i)} className="h-1 rounded-full transition-all duration-500"
                  style={{ background: i === slideIndex ? t.btnBg : t.borderSubtle, width: i === slideIndex ? 32 : 12 }} />
              ))}
            </div>
          </div>
        </div>
      ...
      <div className="flex-1 flex items-center justify-center p-6 sm:p-10 lg:pl-16 xl:pl-20">
        <div className="w-full max-w-md">
          ...
          <a href="/" className="inline-flex items-center gap-1.5 text-xs font-medium mb-8 transition-colors hover:opacity-80" style={{ color: t.textSecondary }}>
          ...
          <motion.div key={mode} initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
            ...
            <div className="space-y-4">
              {mode === "register" && (
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-xs font-semibold flex items-center gap-1.5 mb-2" style={{ color: t.textLink }}>
                      <User className="w-3.5 h-3.5" /> Nome
                    </label>
                    <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="João"
                      className="w-full rounded-xl px-4 py-3 text-sm outline-none transition-all focus:ring-2 focus:ring-foreground/15"
              ...
              <div>
                ...
                <div className="relative">
                  <input type={showPassword ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="••••••••"
                    className="w-full rounded-xl px-4 py-3 pr-12 text-sm outline-none transition-all focus:ring-2 focus:ring-foreground/15"
                    style={{ background: t.inputBg, border: `1px solid ${t.inputBorder}`, color: t.textPrimary }} />
            ...
            <p className="text-center text-xs mt-6" style={{ color: t.textMuted }}>
              ...
              <button onClick={() => { setMode(mode === "login" ? "register" : "login"); setPassword(""); }}
                className="font-semibold transition-colors hover:opacity-80" style={{ color: t.textLink }}>
